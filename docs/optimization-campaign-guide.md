...
- **`regimes`** — a conjunction that must hold in *every* named regime, for
  ...
  away. Example: `{id: crashleg, metric: improvement_crashleg_pct, op: ">",
...
- **`type`** is `numeric` or `choice` — a domain question, not a statistics
  ...
  (`tp_low` in a threshold sweep, `queue_count` in `{2,4,8,16}`).
...
### 3.1 Multi-level grid — `candidate-threshold-robustness`
...
optimization:
  ...
  factors:
    ...
    - id: F4
      ...
      # `> 0`. `trailing_stop_events > 0` would be trivially true -- it
      ...
      manipulation: {observable: config.trailing_stop_threshold, op: "==",
...
- **`trailing_stop` is `choice`, not `numeric`**, because `off` sits in a
...
- **R5 encodes a cross-factor inertness relation** — `min_peak` must do
  nothing when `trailing_stop=off`. That is a real bug class in threshold
...
```yaml
response:
  primary: {metric: train_annualized_return, direction: maximize}
  held_out: [train_annualized_return]     # same metric, hard rejected
```
...
`symphony-generation` leakage class, where a held-out generalization check
...
```yaml
response:
  held_out: [held_out_annualized_retrun]   # typo: "retrun"
```
...
response:
  primary: {metric: train_annualized_return, direction: maximize}
  held_out: [held_out_annualized_return]   # spelled to match the target's actual field
```

...
T10's validator, once working, rejected the spec's §11 example on a cross-field rule:
  (a) factor F4's manipulation predicate was {telemetry.trailing_stop_events, ">", 0}
...
Ruling: fix the SPEC. F4's predicate now compares config.trailing_stop_threshold

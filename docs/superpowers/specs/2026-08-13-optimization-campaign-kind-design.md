...
**`alert-threshold-robustness` is a hand-rolled full factorial.** Its
`controllable_knobs` enumerate `escalate_low ∈ {0.015,0.020,0.025,0.030,0.040}`,
`escalate_high ∈ {6 values}`, `severity_boundary ∈ {3}`, `decay_guard_threshold ∈ {5}`,
`decay_guard_min_peak ∈ {3}` — a 5×6×3×5×3 = **1350-cell grid**. A
...
scalars.** `alert-threshold-robustness` requires beating the recorded
baseline in the bursty stretches AND the steady ones AND holding across ≥4 of
5 held-out blocks.
...
`holdout-selection` declares `train_score` as "the selection key"
and `held_out_score: (generalization check, NOT the selection
...
`max_regret_pct <= 60%`, `budget_limit`, and `trigger_rate_pct` trending to 0 ("a
...
optimization:
  response:
    ...
    constraints:            # admissibility; violated => config infeasible
      - {metric: continuous_max_regret, op: ">=", value: -0.60}
...
* `factor_nomination` → read at `verify`, when the model proposes factors and
  ...
  and "try parameters in a declared configuration" belong.
...
| Held-out metric present in a fitting input | **Hard-fail.** Prevents the `holdout-selection` leakage class. |
...
3. Four worked end-to-end examples drawn from the real corpus:
   * multi-level grid → `alert-threshold-robustness` (1350 cells → ~50 runs)

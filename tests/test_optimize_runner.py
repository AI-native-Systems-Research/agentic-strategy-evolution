...
def test_held_out_metric_recorded_but_absent_from_response_at_every_level():
    ...
    # deliberate intent.
    assert math.isclose(outcome.held_out["held_out_return"], 0.42, rel_tol=1e-9, abs_tol=1e-12)
    # Absent from response at the top level ...
    assert "held_out_return" not in outcome.response
...
def test_primary_and_constraint_metrics_present_in_response_absent_from_held_out():
    ...
    response_spec = _response_spec(
        constraints=[{"metric": "drawdown", "op": ">=", "value": -0.5}],
        held_out=["held_out_return"],
    )
    runner = _RecordingRunner({
        0: {
            "throughput": 10.0, "drawdown": -0.1, "held_out_return": 0.42,
            "telemetry": {"L1": "off"},
        },
    })
    ...
    assert math.isclose(outcome.response["drawdown"], -0.1, rel_tol=1e-9, abs_tol=1e-12)
    # ... and are absent from held_out.
    assert "throughput" not in outcome.held_out
    assert "drawdown" not in outcome.held_out

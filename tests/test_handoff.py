...
class TestHandoffExtraction:
    def test_handoff_md_created_when_present(self, tmp_path: Path) -> None:
        raw = _make_raw_design(include_handoff=True)
        _split_design_output(raw, tmp_path)
    ...
    def test_handoff_md_not_created_when_absent(self, tmp_path: Path) -> None:
    ...
    def test_bundle_still_valid_with_handoff(self, tmp_path: Path) -> None:
        raw = _make_raw_design(include_handoff=True)
        _split_design_output(raw, tmp_path)
        bundle = yaml.safe_load((tmp_path / "bundle.yaml").read_text())
        assert bundle["metadata"]["family"] == "test"
        assert len(bundle["arms"]) == 2

    def test_problem_md_not_polluted_by_handoff(self, tmp_path: Path) -> None:
        raw = _make_raw_design(include_handoff=True)
        _split_design_output(raw, tmp_path)
        problem = (tmp_path / "problem.md").read_text()
    ...
    def test_yaml_fence_in_handoff_does_not_confuse_bundle(self, tmp_path: Path) -> None:
        ...
        _split_design_output(raw, tmp_path)
        parsed_bundle = yaml.safe_load((tmp_path / "bundle.yaml").read_text())
...
class TestHandoffRegexTolerance:
    def test_heading_case_insensitive(self, tmp_path: Path) -> None:
        raw = _make_raw_design(include_handoff=True, handoff_heading="## HANDOFF")
        _split_design_output(raw, tmp_path)
        assert (tmp_path / "handoff.md").exists()

    def test_heading_level_3(self, tmp_path: Path) -> None:
        raw = _make_raw_design(include_handoff=True, handoff_heading="### Handoff")
        _split_design_output(raw, tmp_path)
        assert (tmp_path / "handoff.md").exists()

    def test_no_false_positive_on_handoff_notes(self, tmp_path: Path) -> None:
